...
export class DialogPage implements OnInit {
    ...
    async reportMessage(message, index) {
        ...
        const alert = await this.alertCtrl.create({
            header: 'דווח על תוכן פוגעני',
            message: 'האם ברצונך לדווח על תוכן זה כפוגעני',
            buttons: [
                {
                    text: 'כן',
                    handler: () => {
    ...
    private reportMessageHandler(message, index) {
        ...
        const params = {
            contact: {
                email: this.user.id,
                text: `משתמש ${this.user.id} ביקש לדווח על הודעה ${index} כפוגענית `,
                subject: 'פוגעני באתר פולידייט'
            }
        };
